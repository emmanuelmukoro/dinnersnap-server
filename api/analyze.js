...
// run a function w/ timeout and abort signal
...
async function callVision(imageBase64) {
  if (!GCV_KEY) {
    // don't crash if GCV key missing, just return empty
...
async function spoonacularRecipes(pantry, prefs) {
  ...
  // we let ready time flex a bit beyond prefs.time
  ...
  // use explore to shake variety: just shuffle in final stage,
  // not in query, to avoid rate-punishing the API
  let j = {};
  try {
  ...
  const filtered = raw.filter((it) => {
    ...
    // at least 1 pantry-specific match in title or ingredients
    if (
      specific.length > 0 &&
  ...
  // shuffle for a tiny bit of variety if explore bumped
  const arr = [...scored];
  if (prefs.explore) {
...
async function llmRecipe(pantry, prefs) {
  ...
  try {
    ...
    // normalise and dedupe ingredients later on client side too
...
function emergencyRecipe(pantry) {
  ...
  const title =
    (titleBits.length ? titleBits.join(" ") : "Pantry") +
    " Savoury Skillet";
  ...
  const baseIngs = [
    ...
    },
    {
      ...
      have:
        pantry.includes("salt") || pantry.includes("black pepper"),
    },
  ];

  const uniqPantry = pantry.map((p) => ({
    name: p,
    have: true,
  }));
...
export default async function handler(req) {
  ...
  // watchdog so we never blow vercel timeout
  ...
  const main = async () => {
    ...
    // 3. Gather, prefer spoon but include llm
    let spoonList = [];
    let llmList = [];
    ...
    try {
      ...
      // shouldn't really happen because we catch each
    }

    let combined = [];

    if (spoonList.length > 0) {
      combined = spoonList;
    }
    if (combined.length < 3 && llmList.length > 0) {
      // add LLM recipe if we still have room
